...
void UtilityClass::format_time(char* buffer, const size_t buffer_size, int seconds, int frames, bool always_minutes)
{
    ...
    if (h > 0)
    {
        ...
        vformat_buf(buffer, buffer_size,
            loc::gettext(frames == -1 ? ID("{hrs}:{min|digits=2}:{sec|digits=2}", STR_FMT_HMMSS) : ID("{hrs}:{min|digits=2}:{sec|digits=2}.{cen|digits=2}", STR_FMT_HMMSSCC)),
    ...
    else if (m > 0 || always_minutes || frames == -1)
    {
        /* M:SS / M:SS.CC */
        vformat_buf(buffer, buffer_size,
            loc::gettext(frames == -1 ? ID("{min}:{sec|digits=2}", STR_FMT_MSS) : ID("{min}:{sec|digits=2}.{cen|digits=2}", STR_FMT_MSSCC)),
    ...
    else
    {
        /* S.CC */
        vformat_buf(buffer, buffer_size,
            loc::gettext(ID("{sec}.{cen|digits=2}", STR_FMT_SCC)),
            "sec:int, cen:int",
            s, frames * 100 / 30
        );
    }
