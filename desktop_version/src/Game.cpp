...
static void im1_instructions_textbox1(textboxclass* THIS)
{
    ...
    #ifdef __NDS__
    String_ID string_id = (String_ID)(STR_IM1_FLOOR_VITELLARY + ((game.lastsaved >= 2 && game.lastsaved <= 5) ? game.lastsaved - 2 : 4) + (graphics.flipmode ? 5 : 0));
    #else
...
static void im1_instructions_textbox2(textboxclass* THIS)
{
    ...
    // Intermission 1 instructional textbox, depends on last saved
    #ifdef __NDS__
    String_ID string_id = (String_ID)(STR_IM1_NOT_FLOOR_VITELLARY + ((game.lastsaved >= 2 && game.lastsaved <= 5) ? game.lastsaved - 2 : 4) + (graphics.flipmode ? 5 : 0));
    #else
    const char* floorceiling = graphics.flipmode ? "ceiling" : "floor";
    const char* crewmate;
    switch (game.lastsaved)
